...
async function seedDatabase(prisma: PrismaClient) {
    ...
    for (const user of createdUsers) {
        const categories = Array.from(
            {length: SEED_COUNT.CATEGORIES_PER_USER},
            () => {
        ...
        try {
            ...
            console.error(
                `Error creating categories for user ${user.id}:`,
                error,
            );
            throw error;
        }
    ...
    for (const category of categories) {
        try {
            ...
            const rootPosts = Array.from(
                {length: Math.ceil(SEED_COUNT.POSTS_PER_CATEGORY / 3)},
                () => {
            ...
            console.error(
                `Error creating posts for category ${category.id}:`,
                error,
            );
            throw error;
        }
    ...
    for (const post of posts) {
        try {
            ...
            const rootComments = Array.from(
                {length: SEED_COUNT.COMMENTS_PER_POST},
                () => ({
            ...
            for (const parent of parentComments) {
                const replies = Array.from(
                    {length: SEED_COUNT.REPLIES_PER_COMMENT},
                    () => ({
                        content: faker.lorem.paragraph(),
                        userId: faker.helpers.arrayElement(createdUsers).id,
                        postId: post.id,
            ...
            console.error(
                `Error creating comments for post ${post.id}:`,
                error,
            );
            throw error;
        }
